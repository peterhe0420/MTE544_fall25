...
class decision_maker(Node):
    ...
    def __init__(self, publisher_msg, publishing_topic, qos_publisher, goalPoint, rate=10, motion_type=POINT_PLANNER):
        ...
        if motion_type == POINT_PLANNER:
            self.controller=controller(klp=2, kli=0.5, klv=0.2, kap=2.2, kai=0.1, kav=0.2)
            self.planner=planner(POINT_PLANNER)    
    
    
        elif motion_type==TRAJECTORY_PLANNER:
            self.controller=trajectoryController(klp=2, kli=0.5, klv=0.2, kap=2.2, kai=0.1, kav=0.2)
    ...
    def timerCallback(self):
        ...
        if type(self.goal) == list:
            # For trajectory planner, check if we've reached the final goal
            final_goal = self.goal[-1]
            linear_error = calculate_linear_error(self.localizer.getPose(), final_goal)
            reached_goal = linear_error < 0.01  # 10cm tolerance
        else: 
            # For point planner, check if we've reached the single goal
...
def main(args=None):
    ...
    if args.motion.lower() == "point":
        ...
        DM = decision_maker(
            ...
            publishing_topic="/cmd_vel",
            qos_publisher=odom_qos,
            goalPoint=[],  # Empty for trajectory planner
            rate=10,
